...
(()=>{
  ...
  function syncCategoryButtons(){
    ...
    buttons.forEach(btn=>{
      ...
      if(!onclick.includes('openCategoriesModal'))return;
      const context=[btn.id||'',btn.className||'',...Array.from(btn.closest('[id]')?[btn.closest('[id]').id]:[]),...Array.from(btn.closest('[class]')?[btn.closest('[class]').className]:[])].join(' ').toLowerCase();
      const articleContext=/artic|product|inventario/.test(context);
      btn.style.display=articleContext?'':'none';
      btn.setAttribute('aria-hidden',articleContext?'false':'true');
    });
  }
  function initCategories(){if(!window.db)return;ensureCategories();try{save()}catch(e){console.warn('No se pudieron guardar las categorías iniciales',e)};patchProductModal();syncCategoryButtons()}
  ...
  // No se usa MutationObserver ni un intervalo permanente: se actualiza únicamente después de acciones del usuario.
  document.addEventListener('click',()=>setTimeout(()=>{patchProductModal();syncCategoryButtons()},0),true);
})();
...
(()=>{
  ...
  function beginPending(){if(pendingWindow&&!pendingWindow.closed)return;const d=getDb();pendingSalesCount=Array.isArray(d?.sales)?d.sales.length:0;pendingStartedAt=Date.now();try{pendingWindow=window.open('about:blank','_blank','width=460,height=850')}catch(e){pendingWindow=null}if(!pendingWindow){toast('El navegador bloqueó el ticket. Permite ventanas emergentes para VENTARA POS.');return}showPreparing(pendingWindow);pendingTimer=setInterval(()=>{if(!pendingWindow||pendingWindow.closed){clearPending();pendingWindow=null;return}if(finishPending())return;if(Date.now()-pendingStartedAt>20000){clearPending();try{pendingWindow.close()}catch(e){}pendingWindow=null}},120)}
  ...
  window.printTicket=function(id,autoPrint=true){const d=getDb();if(!d)return;const s=(d.sales||[]).find(x=>x.id===id);if(!s)return toast('No se encontró la venta');if(pendingWindow&&!pendingWindow.closed){const w=pendingWindow;pendingWindow=null;clearPending();if(writeTicket(w,s,true))return}const w=window.open('about:blank','_blank','width=460,height=850');if(!w)return toast('El navegador bloqueó la ventana del ticket. Permite ventanas emergentes para VENTARA POS.');writeTicket(w,s,autoPrint)};
